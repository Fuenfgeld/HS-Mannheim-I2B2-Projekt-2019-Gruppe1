...
from backend.abfragenLogik.formatted import strings_sql
...
class SQLTemplates:
    ...
    def search_code_with_df(self, df):
        for n in range(0, len(df)):
            if self.ICD9 in df.loc[n].values[0]:
                break
        return df.loc[n].values[0]


    def get_patient_with_icd(self, icd):
        df2 = pd.read_sql(strings_sql.build_SQL_icd(icd), con=database.engine)
        return df2
    ...
    def anzahlPatEinKriteriumEltern(self, c_fullname):
        ...
        df3 = pd.DataFrame()
        for n in range(0, len(df)):
            if self.ICD10 not in df.loc[n].values[0]:
